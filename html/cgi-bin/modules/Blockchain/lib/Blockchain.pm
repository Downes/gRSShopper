...
sub new {
	...
	# Retrieve stored version of the blockchain from file
	...
	if ($#{$self->{chain}} < 0) {
      print "Initializing a new genesis block. ";
...
sub new_block {
	...
	$previous_hash ||= "None";
  my $index = $#{$self->{chain}}+1;
	my @transactions = $self->{current_transactions};
  push @{$self->{old_transactions}},@transactions;
  $self->{current_transactions} = [];
...
sub new_transaction {
	 ...
	 push @{$self->{current_transactions}},$data;
...
sub last_block {
	...
	# - Find a number p' such that hash(pp') contains leading 2 zeroes, where p is the previous p'
...
sub valid_proof {
 ...
 my ($self,$last_proof,$proof) = @_;
	...
	my $test = substr $digest, 0, 2;
	if ($test eq "00") { return 1;} else { return 0;}
...
# Perform a mining operation to create a new block
sub mine {
	my ($self) = @_;
	my $node_identifier = 1;
  if ($#{$self->{current_transactions}} < 0) { die "Not willing to create a block with 0 transactions"; }

	# We run the proof of work algorithm to get the next proof...
	my $last_block = $self->last_block() || die "Last block not found in blockchain.pm mine()";
	my $last_proof = $self->{proof};
	my $proof = $self->proof_of_work($last_proof);

	# We must receive a reward for finding the proof.
	# The sender is "0" to signify that this node has mined a new coin.
	$self->new_transaction({sender=>0,recipient=>$node_identifier,amount=>1});
	my $previous_hash = $self->hash($last_block);
	my $block = $self->new_block($proof,$previous_hash);
...
sub open {
	...
	use Cwd qw(abs_path);
	my $datadir = dirname(dirname abs_path $0) . '/cgi-bin/';
	my $blockchain_file = $datadir."data/blockchain.json";


	unless (-e $blockchain_file) { # Initialize Bloackchain file if it doesn't exist
    open OUT,">$blockchain_file" or die "Could not initialize $blockchain_file ";
		print OUT "{}\n" or die "Could not initialize $blockchain_file "; ;
		close OUT;
  }


	open(my $fh, "$blockchain_file") or die "Could not open $blockchain_file for read";
