...
#   Updated March 11, 2022 by Stephen Downes to enable support for 'a' tags
#   See handler_url()
...
my @specials = (
		...
		[chr(206), '\\^I'          ], #I
		[chr(207), '\\"I'          ], #�
		[chr(208), '{D\\hspace*{-1.7ex}-\\hspace{.9ex}}'], #�*
		[chr(209), '\\~N'          ], #�
		[chr(210), '\\`O'          ], #�
		[chr(211), '\\\'O'         ], #�
		[chr(212), '\\^O'          ], #�
...
# HTML input form: <a href="foo">Bar</a>
# Latex output form: \url{foo}{Bar}
sub url_handler {
    my($html_element,@tex) = @_; 
	return urlify($html_element->{href})."\{".texify($html_element)."\}";
}
...
sub texify {
    my ($parent_element,$parent_attribute) = @_;
	...
	if(ref $html_element){
		...
		my $href = $html_element->attr('href');
		...
		# Otherwise, just texify the contents;
			...
			$output .= texify($html_element,$href);
	    }
	} else {
	    # Otherwise, it's just a string 

		if ($parent_attribute) { $html_element = $parent_attribute." ".$html_element; }
