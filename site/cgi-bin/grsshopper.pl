...
package gRSShopper::Site;
  ...
  sub __home {
	...
	# Assumes directory structure is /var/www/$host/html/cgi-dir/data/multisite.txt
  	...
  	if ($ENV{'HTTP_HOST'}) { 
		$self->{st_host} = $ENV{'HTTP_HOST'};      				 # HTTP
  	} elsif ($ARGV[0]) { 
		$self->{context} = "cron"; 
		$self->{st_host} = $ARGV[0];
	} else { 
		die "Cannot find website host from HTTP or Cron input.";
	}
	my $host = $self->{st_host}; my $hostdir;

	# Define site content directories
	...
	$self->{st_urlf} = $hostdir;							# save as Site data

	# Define CGI and data directories
	$self->{script} = $0; 
	use File::Basename;
	$self->{st_cgif} = dirname(__FILE__)."/";
