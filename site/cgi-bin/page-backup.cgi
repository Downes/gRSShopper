...
# ( assumes page.cgi?$table=$id )
	...
	print "Error: No data type provided to list.";
	exit;
...
sub show_page {
	...
	# Find page by id, or title, if applicable
...
#	}
	...
	$page->{page_content} =
		&db_get_template($dbh,"responsive_header",$record->{page_title}) . 
		$output .
		&db_get_template($dbh,"responsive_footer",$record->{page_title});
