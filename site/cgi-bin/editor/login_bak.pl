...
#       use local::lib; # sets up a local lib at ~/perl5
    ...
    my $password = shift;
# normalize password input to raw bytes
      $password =~ s/\r?\n\z//;          # strip CR/LF if present
      utf8::downgrade($password, 1);     # ensure bytes (not UTF-8 chars)

    # Generate a salt if one is not passed
    
      my $salt = shift || &_salt();
      # bcrypt requires exactly 16 octets (bytes), not characters
      utf8::downgrade($salt, 1);   # make it bytes if it was UTF-8
...
sub _check_password {
print "Content-type: text/html\n\n";
print "Ckecing password...<br>";
    my ($plain_password, $hashed_password) = @_;
    $plain_password =~ s/\r?\n\z//;
    utf8::downgrade($plain_password, 1);


    # expected format: <16-char-salt>-<base64hash>
    my ($salt, $rest) = split('-', $hashed_password, 2);

    utf8::downgrade($salt, 1) if defined $salt;

    # If hash isn't in bcrypt format (or has legacy salt length), treat as non-match
    return 0 unless defined $salt && defined $rest && length($salt) == 16;

    my $test = _encrypt_password($plain_password, $salt);
print "$test eq $hashed_password ?";
    return $test eq $hashed_password ? 1 : 0;
}
